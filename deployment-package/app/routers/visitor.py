...
async def check_in_visitor_with_image(
    ...
    def send_sms_background(visitor_id: int, person_to_meet: str, visitor_name: str, 
                           mobile: str, email: Optional[str], company: Optional[str], 
                           reason: str, warehouse: Optional[str], visit_date: Optional[str] = None,
    ...
    # Add background task (non-blocking)
    logger.info(f"[SMS] Adding background task to send SMS for visitor {new_visitor.id}, person_to_meet: {person_to_meet}")
    background_tasks.add_task(
        send_sms_background,
        new_visitor.id,
        person_to_meet,
